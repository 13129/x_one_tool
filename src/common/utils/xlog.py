    ...
    async def async_trace_add_log_record(self, request: Request, event_des: str = None, msg_dict: dict = None,
        ...
        if hasattr(request, 'traceid'):
            ...
            if not remarks:
                context.pop('remarks')
            if not msg_dict:
                context.pop('msg_dict')
            try:
                log_msg = dict_to_json_ensure_ascii(context)
                self.log.info(log_msg)
            except (TypeError, OverflowError, ValueError, UnicodeEncodeError) as e:
                self.log.info(
                    getattr(request, 'traceid') + ':index:' + str(
                        getattr(request, 'trace_links_index')) + ':log info write error!!!' + f'{e}')
